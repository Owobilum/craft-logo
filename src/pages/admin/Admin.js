...
const Admin = () => {
  ...
  const handleGetOrders = useCallback(async (pageChange)=>{
    let page
    if(pageChange==="next"){
      page = currentPage+1
    ...
    try{
      setIsLoading(true)
      const response = await fetch(`https://craft-logo-backend.herokuapp.com/api/all-orders?page=${page}&${params}`)
      const {data:{data,current_page,last_page}} = await response.json()
      setOrders(data)
  ...
  const handleVerifyPasscode = useCallback(async ()=>{
    ...
    if(isSubmitted){
      setIsLoading(true)
      try{
        const response = await fetch('https://craft-logo-backend.herokuapp.com/api/verify_passcode',{
          method:'POST',
          headers:{
  ...
  const handleMarkCompleted = async(id)=>{
    try{
      setIsLoading(true)
      const response = await fetch(`https://craft-logo-backend.herokuapp.com/api/complete_order/${id}?passcode=${passcode}`)
      if(response.status === 200){
    ...
    if(isVerified){
      ...
      return (
        <div className={styles.root}>
          ...
          {message && <div className='alert alert-success mb-3'><div className='container'>{message}</div></div>}
            ...
            <div style={{padding: '0 5%'}}>
              ...
              <p>
                <button 
                  ...
                  style={filter==="all"?{}:
                  {backgroundColor: '#fff',color: '#000',outline: "1px solid black"}}
                  name="all"
              ...
              <div style={{overflowX:'auto'}}>
                <table>
                  ...
                  <tbody>
                    {
                      orders?.map(({order:{name,tagline, email, phone,updated_at,id,amount},status })=>{
              ...
              {/* FOOTER */}
              <div style={{marginTop: 16,display:'flex',alignItems:'flex-end',flexDirection:'column'}}>
